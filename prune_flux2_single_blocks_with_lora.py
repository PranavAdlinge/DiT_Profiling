...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Load a Flux2 transformer, attach and fuse a LoRA adapter from config and "
            "checkpoint, prune selected single transformer blocks, and save a checkpoint "
            "without separate LoRA weights."
        )
    )
    parser.add_argument(
        "--dit-pretrained-path",
        type=Path,
        required=True,
        help="Path passed to Flux2Transformer2DModel.from_pretrained(...).",
    )
    parser.add_argument(
        "--lora-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Config path consumed by config.load_config(...).",
    )
    parser.add_argument(
        "--output-weights",
        type=Path,
...
def prune_single_transformer_blocks(transformer, skip_blocks_1_based):
    original_count = len(transformer.single_transformer_blocks)
    ...
    kept_blocks = [
        ...
        if index not in skip_zero_based
    ]
    transformer.single_transformer_blocks = torch.nn.ModuleList(kept_blocks)

    if hasattr(transformer.config, "num_single_layers"):
        transformer.config.num_single_layers = len(kept_blocks)

    return original_count, len(kept_blocks)
...
def main():
    ...
    original_single_layers = len(transformer.single_transformer_blocks)
    ...
    original_count, pruned_count = prune_single_transformer_blocks(transformer, skip_blocks)
    ...
    save_json(output_config, dict(transformer.config))
    ...
    print(f"num_single_layers: {original_count} -> {pruned_count}")
